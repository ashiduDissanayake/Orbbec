...
public:
    ...
    std::vector<Ball> detect(const cv::Mat &colorFrame, 
        ...
        // Scale from YOLO output to original image
...
private:
    Ball createBallFrom2D(const cv::Rect &colorBox, float confidence,
        ...
        // Map from color resolution to depth resolution
      ...
      if (rawDepth > 50) {
          ...
          // CRITICAL FIX: The ball is close (15-20cm) so radius will be TINY
          // Real calculation: (41px * 0.15m) / 554.3 = 0.011m = 1.1cm
          
          // ACCEPT SMALLER RADII - ball is very close to camera!
          bool validRadius = (ball.radius3D >= 0.005f && ball.radius3D <= 0.50f);  // 0.5cm to 50cm
          bool validDepth = (ball.position3D[2] >= 0.05f && ball.position3D[2] <= 5.0f);   // 5cm to 5m
          
          if (!validRadius || !validDepth) {
    ...
    uint16_t sampleDepth(float centerX, float centerY, float radius,
        ...
        // Sample around circle (median filter)
